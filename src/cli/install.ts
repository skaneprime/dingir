...
const PATH_TO_PAKAGE_JSON = `${process.cwd()}/package.json`;
const readPkgJson = () => readFile(PATH_TO_PAKAGE_JSON, 'utf8');
...
program
	...
	.action(async (library: string | undefined, options: { debug?: boolean | string }) => {
		...
		for(const library of libraries) {
			...
			if(pkgJson.dingir)
				if(pkgJson.dingir.libraries) 
					pkgJson.dingir.libraries[lib] = fetchedLib.version;
				else 
					pkgJson.dingir.libraries = { [lib]: fetchedLib.versions }; 
			else 
				pkgJson.dingir = {
					libraries: {
			...
			await writeFile(PATH_TO_PAKAGE_JSON, JSON.stringify(pkgJson, null, '  '));
