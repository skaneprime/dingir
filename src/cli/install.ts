...
async function readLibsFromPKGJson() {
	...
	return json?.dingir?.libraries as { [key: string]: string } | undefined;
}
...
program
	...
	.action(
		async (
			...
			options: { debug?: boolean | string; latest?: boolean },
		) => {
			...
			for (const library of libraries) {
				...
				const fetchedLib = await api.libs.getLib(lib, options.latest ? undefined : ver);
